...
####Stock
    ...
    *
    * This will throw a pierresilva\Inventory\Exceptions\NotEnoughStockException if there is not enough
    */
    ...
    *
    * This will throw a pierresilva\Inventory\Exceptions\InvalidQuantityException if it is not valid
    */
...
If an item already has an SKU, and `createSku($code)` is called, then an `pierresilva\Inventory\Exceptions\SkuAlreadyExistsException`
will be thrown.
...
Occurs when you're trying to take more stock than available. Use a try/catch block when processing take/remove actions:
    ...
    } catch(pierresilva\Inventory\Exceptions\NotEnoughStockException)
    {
...
Occurs when a stock of the item already exists at the specified location. Use a try/catch block when processing move actions:
    ...
    } catch(pierresilva\Inventory\Exceptions\StockAlreadyExistsException)
    {
...
occurring, or you can use a try/catch block when processing actions involving quantity:
    ...
    } catch(pierresilva\Inventory\Exceptions\InvalidQuantityException)
    {
