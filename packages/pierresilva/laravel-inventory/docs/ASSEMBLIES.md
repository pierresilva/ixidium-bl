...
The exception `pierresilva\Inventory\Exceptions\InvalidPartException` is thrown when you try to add a part to an assembly
...
As such when managing stock quantities, you will receive an `pierresilva\Inventory\Exceptions\InvalidQuantityException` if
