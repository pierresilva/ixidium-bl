...
class CrudMigrationCommand extends GeneratorCommand
{
    ...
    protected function getPath($name)
    {
        ...
        if ($this->option('module-name')) {
            return str_replace('\\', '/', 'app/Modules/' . $this->option('module-name') . '/Database/Migrations/' . $datePrefix . '_create_' . $name . '_table.php');
        }
    ...
    protected function buildClass($name)
    {
        ...
        $softDeletesSnippets = '';
        if ($softDeletes == 'yes') {
            $softDeletesSnippets = "\$table->softDeletes('deleted_at', 3);\n" . $tabIndent . $tabIndent . $tabIndent;
        }
