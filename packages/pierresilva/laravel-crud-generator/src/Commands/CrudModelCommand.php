...
class CrudModelCommand extends GeneratorCommand
{
    ...
    protected $signature = 'crud:model
                            ...
                            {--model-namespace= : The name of the table.}
    ...
    protected function buildClass($name)
    {
        ...
        $modelNamespace = $this->option('module-name') ? $moduleNamespace . 'Models' : $this->option('model-namespace');
        if (!empty($primaryKey)) {
            $primaryKey = <<<EOD
/**
...
EOD;
        ...
        $ret->replaceModelNamespace($stub, $modelNamespace);
    ...
    public function replaceModelNamespace(&$stub, $modelNamespace)
    {
     ...
     * @param array $argsString args for the relationship function
    ...
    protected function getPath($name)
    {
        if ($this->option('module-name')) {
            return base_path() . '/app/Modules/' . $this->option('module-name') . '/Models/' . str_replace('\\', '/', $this->argument('name')) . '.php';
        }
        return base_path() . '/app/' . str_replace('\\', '/', $this->argument('name')) . '.php';
    }
}
