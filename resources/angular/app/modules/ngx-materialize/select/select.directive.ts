...
export class MzSelectDirective extends HandlePropChanges implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    // setTimeout(() => {
    //   this.renderer.invokeElementMethod(this.selectElement, 'update');
    // });

  }

  ngOnDestroy() {
    this.renderer.invokeElementMethod(this.selectElement, 'formSelect', ['destroy']);
  ...
  initMaterialSelect() {
    this.renderer.invokeElementMethod(this.selectElement, 'formSelect');
  }

  /**
   * Trigger the native change event from select element instead of the any.
  ...
  handleDOMEvents() {
    this.inputElement.on('blur focus', (event: Event) => {
      const customEvent = document.createEvent('CustomEvent');
