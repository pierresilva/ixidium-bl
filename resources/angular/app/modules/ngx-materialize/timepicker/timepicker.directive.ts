...
export class MzTimepickerDirective extends HandlePropChanges implements OnInit, OnDestroy {
  ...
  initElements() {
    this.inputContainerElement = $(this.elementRef.nativeElement).parent('.input-field') as any;
    this.inputElement = $(this.elementRef.nativeElement) as any;
    this.labelElement = this.createLabelElement() as any;
  }
  ...
  initTimepicker() {
    ...
    this.renderer.invokeElementMethod(this.inputElement, 'timepicker', [this.options]);

    if (this.ngControl) {
      // set ngControl value according to selected time in timepicker
      this.inputElement.on('change', (event: any) => {
